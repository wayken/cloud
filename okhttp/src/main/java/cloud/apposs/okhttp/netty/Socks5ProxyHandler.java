...
public class Socks5ProxyHandler extends ProxyHandler {
    ...
    // 是否使用 SOCKS v5 时代理 DNS 查询
    private final boolean proxyDns;

    private String decoderName;
    private String encoderName;

    public Socks5ProxyHandler(SocketAddress proxyAddress) {
        this(proxyAddress, false, null, null);
    }

    public Socks5ProxyHandler(SocketAddress proxyAddress, boolean proxyDns) {
        this(proxyAddress, proxyDns, null, null);
    }

    public Socks5ProxyHandler(SocketAddress proxyAddress, boolean proxyDns, String username, String password) {
        super(proxyAddress);
    ...
    private void sendConnectCommand(ChannelHandlerContext ctx) throws Exception {
        ...
        String rhost;
        if (proxyDns || raddr.isUnresolved()) {
