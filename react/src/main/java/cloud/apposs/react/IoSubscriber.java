...
public abstract class IoSubscriber<T> implements IoObserver<T>, IoSubscription {
    ...
    // 取消订阅时的回调列表
    private List<IoRunnable> unsubscribeListeners;
    ...
    public final void add(IoSubscription subscription) throws Exception {
    ...
    public void unsubscribe() throws Exception {
        ...
        notifyUnsubscribeListeners();
    }

    @Override
    public void addOnUnsubscribe(IoRunnable onUnsubscribe) throws Exception {
        ...
        synchronized (this) {
            if (unsubscribeListeners == null) {
                unsubscribeListeners = new ArrayList<IoRunnable>();
            }
            unsubscribeListeners.add(onUnsubscribe);
        }
    }

    protected void notifyUnsubscribeListeners() throws Exception {
        List<IoRunnable> listeners;
        synchronized (this) {
            listeners = unsubscribeListeners;
            unsubscribeListeners = null;
        }
        if (listeners != null) {
            for (IoRunnable listener : listeners) {
                listener.call();
            }
        }
    }
