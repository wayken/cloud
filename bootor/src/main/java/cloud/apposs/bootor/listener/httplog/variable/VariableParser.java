...
public class VariableParser extends AbstractVariableParser<BootorHttpRequest, BootorHttpResponse> {
    ...
    protected void finalizeFormatter(String option) {
        if (option.startsWith("http_")) {
            ...
            // 所有日志项都没匹配到，那就直接文本输出
            variableList.add(new LiteralVariable<BootorHttpRequest, BootorHttpResponse>("$" + option));
        }
    }
}
